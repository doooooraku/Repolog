...
- [`android_build.md`](./android_build.md) — APK ビルド手順の詳細
- [`android_debug.md`](./android_debug.md) — logcat / pidcat の詳細
- [`testing.md`](../testing/testing.md) — Jest / E2E テスト手順
- [`git_workflow.md`](../workflow/git_workflow.md) — Issue → PR → Merge のフロー
...
> **詳細手順は** [`android_build.md`](./android_build.md) を参照。
...
### Q3. Expo Dev Client の位置付け

現在は **expo-dev-client を導入済み** で、日常の開発は Dev Build + Metro ホットリロードで行っている。

| 観点 | Preview Build（APK） | Dev Build（expo-dev-client） |
|------|----------------------|-------------------------------|
| 初回セットアップ | `pnpm build:android:apk:local` | `--profile development` でビルド |
| JS 変更時 | 再ビルドが必要 | Metro ホットリロード（1-3秒） |
| ネイティブ変更時 | APK 再ビルド → 再インストール | 同じ |
| 利点 | 本番に近い動作確認 | ホットリロード、DevTools連携 |

**現在の使い分け:**
- **日常開発:** Dev Build + Metro（ワークフロー A）
- **リリース前確認:** Preview Build（ワークフロー B）

詳細は [`dev_vs_preview_builds.md`](./dev_vs_preview_builds.md) と [`android_debug.md`](./android_debug.md) セクション9 を参照。
