# 4_実装ルール.md（Repolog）
目的：Repologの実装を「迷わず」「ブレずに」「テストで守れる」ようにする
...
- `src/features/`：機能単位の実装（例：reports）
...
## 2. 命名ルール（迷子を減らす）
### 2.1 ファイル/フォルダ
- Reactコンポーネント：`PascalCase.tsx`（例：`ReportCard.tsx`）
...
- なぜ5：ルールが「日常のチェック」に組み込まれていないから（運用として定着しない）
