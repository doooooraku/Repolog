...
Repolog を **「仕様→Issue→実装→テスト→PR→マージ→リリース」** のサイクルで回し続け、
...
### 工程W-08：ローカル検証（verify）

* **トリガーキー**：コミットを出す前
* **作業内容**：`pnpm verify` を実行（5ゲートを順に通す）
* **INPUT**：変更後のコード
* **OUTPUT**：なし
* **完了条件**：5ゲート全てが通る
* **担当**：Codex
...
```bash
# 一括実行（CI と同じチェックをローカルで）
pnpm verify

# 内部で以下を順に実行:
# 1. pnpm lint         — ESLint（コード品質）
# 2. pnpm type-check   — tsc --noEmit（型安全）
# 3. pnpm test         — Jest（単体テスト）
# 4. pnpm i18n:check   — 未使用/欠落キー = 0（翻訳整合性）
# 5. pnpm config:check — Expo設定検証（iOS暗号化コンプライアンス等）
```

* **なぜ4つ目（i18n:check）が必要か**：
  大量ファイルの自動変更（一括リネーム、キー削除、コード生成等）で
  「型は通るがi18nキーが壊れている」状態が検出できなかった事故が
  発生したため、CIゲートに追加した。
* **なぜ5つ目（config:check）が必要か**：
  iOS暗号化コンプライアンス（ADR-0010）やバンドルIDの整合性を
  自動検証し、ストア審査の不合格を防止するため追加した。
...
DoD（完了の定義）＝「ここまでできたら終わり」という約束。  
DoDに1つでも×があれば完了ではない（次の工程へ進めない）。

* CI（`.github/workflows/*.yml`）が全て成功（lint + test + type-check + i18n:check）
...
* “なぜ”が増えたなら ADR を追加
* i18n キーを追加/削除した場合は `pnpm i18n:check` がパスすること
