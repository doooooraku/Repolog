...
- **コメント（Comment layout）**：PDF写真ページが「1ページ1枚」＋キャプション表示。Free/Pro共通で出力可能。
...
- **Home（タイムライン）**
- **レポート作成（撮影）**
- **レポート編集（メタ情報・コメント・写真一覧）**
- **写真並べ替え／削除／追加（編集内サブ）**
- **PDFプレビュー**
- **課金（Paywall）**
- **設定（言語、テーマ、位置情報ON/OFF、購入復元、バックアップ）**
- **バックアップ（エクスポート／インポート）**
...
- authorName: string | null（作成者名。PDF表紙に表示）
- tags: string[]（複数）
- pinned: boolean
- photos: Photo[]（順序あり）
...
- orderIndex: number（0..n）
- caption: string | null（写真キャプション。UIで200文字制限）
...
- [x] 撮影すると写真がレポートに追加され、編集画面へ遷移できる
- [x] アルバムから複数枚を選び、レポートに追加できる
- [x] Freeは 10枚を超えて追加できない（超える場合は課金モーダル）
- [x] Proは無制限に追加できる（ただし後述の>50警告は出力時）

---

# レポート編集（メタ情報・コメント・タグ・位置・天気）
...
## 作成者名（authorName）
- 設定画面で入力した作成者名がレポートに自動セットされる
- PDF表紙に表示される
- レポートごとに編集可能（null可）

## 受け入れ条件
- [x] レポート名が前回値で埋まり、ユーザーが編集できる
- [x] 天気が4種+なしから選べる（PDFには翻訳文字列で出る）
- [x] コメント4000文字で入力が止まる（残り文字数表示）
- [x] 位置情報がOFFなら権限要求なし＆PDFは `-`
- [x] 位置情報ONでも取得できない場合は `-`
- [x] 位置情報ONの初回でLat/Lngが自動取得される
- [x] 住所が自動入力され、ユーザーが編集できる
...
## 受け入れ条件
- [x] タイムラインが縦スクロールで見返せる
- [x] ピン留めはHome上部に固定表示（”探すコスト”を減らす）
- [x] 削除は取り消し不能のため確認が必須
...
## 受け入れ条件
- [x] レポート名/コメント/タグで検索できる
- [x] 期間/レポート名/タグで絞り込める（期間はプリセット「今週」、テキスト検索で名前/コメント/タグを横断）
- [x] 条件を複数組み合わせても破綻しない（バックエンドは任意組合せ対応済み）

---

# PDFプレビュー & 出力（A4/Letter / 崩れない / 世界向け）
...
- レイアウト：
  - 標準：1ページ2写真（上下2段）
  - コメント：1ページ1写真＋キャプション表示（Free/Pro共通で出力可能）
...
## Free/Pro とプレビューのUX
- プレビュー画面：
  - Free/Pro共通で「標準」「コメント」の両レイアウトを選択・プレビュー・出力可能
  - レイアウトに関するPro制限は撤廃済み（ADR-0011参照）
...
## 受け入れ条件
- [x] Free/Proの権限が即時反映される（購入直後に制限解除）
- [x] 復元ができる
- [x] Proで広告が出ない
- [x] 同意未完了時にFreeでも広告が出ない（同意後に表示）
...
## 受け入れ条件
- [x] Exportでzip（またはフォルダ）として保存できる
- [x] Importで同じ状態を復元できる（並び順、ピン留め、タグも含む）
- [x] schemaVersionが違う場合は丁寧なエラーを出す（壊さない）
- [x] 同じバックアップを再Importしても重複登録しない（ID基準で冪等）
- [x] Import失敗時は transaction rollback で既存データを壊さない
...
- 言語：端末言語に追従（19言語）
- テーマ（外観）：system / light / dark
- 作成者名（authorName）：PDF表紙に表示される名前
- 位置情報をPDFに含める：ON/OFF
...
- （任意）react-native-google-mobile-ads（広告）
...
npx expo start
```
- `expo start`：開発サーバー起動。expo-dev-client（カスタム Dev Build）やシミュレータから動作確認できる


---

## 9. レビュー＆反映ログ（多視点：反映済み）
