...
- 出力成功後、累計回数とプランに応じて OS 標準のレビュー依頼ダイアログが出ることがある（詳細は ADR-0012）
...
> *¹* **計上単位（ADR-0014 より）**: 「月5回」の1回は **PDFファイルが生成完了した時点** でカウントする。
> iOS の共有シートや Android の SAF でユーザーがキャンセルしても、生成済みの1回は消費される。
> これは `expo-sharing` の iOS 実装が共有シートの cancel/share を区別できない制約を吸収するための設計判断。
