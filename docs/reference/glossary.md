...
### 1-1. 機能名
- 意味：ユーザー価値単位の機能
- 例：レポートCRUD、PDFプレビュー/出力、バックアップ、検索・フィルタ

### 1-2. 画面名
- 意味：UI画面の単位
- 例：Home、Report Editor、PDF Preview、Settings
...
### 作成者名（authorName）
- 定義：レポートの作成者名。PDF表紙およびエディタに表示される。スキーマv3で追加。
- 保存先：settingsStore（デフォルト値）/ 各レポートのauthorNameフィールド

### テーマ（Theme）
- 定義：外観設定。system / light / dark の3モード。settingsStoreに保存。
- 同義語：Appearance、外観
...
### 画面（Screen）
- 定義：ユーザーが見るUIの単位。画面名（例：Home、Settings）で表す。
...
- 運用：`pnpm i18n:audit -- --inventory` で棚卸しし、段階削除の対象を決める。
