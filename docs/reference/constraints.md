# Repolog constraints（前提 / 制約 / 不変条件） - Reference

この文書は Repolog の「前提 / 制約 / 不変条件 / 非ゴール」を 1枚に集約した “ルールブック（Reference）” です。
...
- Repolog は **ローカル完結の写真レポートPDFアプリ** とする（自前バックエンドは持たない）
...
### 2-1. 収益の柱（3本）
- 収益は **サブスクリプション（Pro）** / **買い切り（Lifetime）** / **広告（Free向けバナー）** の3本柱
- **全画面広告（インタースティシャル）は採用しない**（体験劣化が大きい）

### 2-2. Free / Pro の不変差分（ここを崩さない）
- Free：1レポート **写真10枚まで**
- Pro：写真 **無制限**
- Free：PDF出力 **月5回まで**（プレビューは無制限）*¹*
- Pro：PDF出力 **無制限**
- Free：表紙に **透かし**（右下に薄く `Created by Repolog`）
- Pro：透かし **なし**
- Free：ホーム画面下部にバナー広告を表示（常時）
- Pro：広告は **一切表示しない**（広告コンポーネントをマウントしない）
- 課金は RevenueCat 経由で行い、**復元（Restore purchases）を提供**する

> *¹* **計上単位（ADR-0014）**: 「月5回」の1回は **PDFファイルが生成完了した時点** でカウントする。OS の保存/共有 UI でユーザーがキャンセルしても、生成済みの1回は消費される。これは `expo-sharing` の iOS 実装が共有シートの cancel/share を JS 層に伝えられない制約を吸収するための恒久ポリシー。詳細は `docs/adr/ADR-0014-pdf-export-count-policy.md`。
