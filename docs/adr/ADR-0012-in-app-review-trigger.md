...
- **手動チェック（iOS 実機）**:
  - ⚠️ **TestFlight ビルドではダイアログは表示されない** — これは Apple の公式仕様で、`SKStoreReviewController.requestReview()` は TestFlight 配布アプリでは一切 UI を出さない（[Apple Developer Forums](https://developer.apple.com/forums/thread/794961) / [Critical Moments](https://criticalmoments.io/blog/skstorereviewcontroller_guide_with_examples) / [react-native-rate#52](https://github.com/KjellConnelly/react-native-rate/issues/52)）。Issue #289 で再発確認済み
  - 検証可能なビルド種別: **App Store Production 配信版** または **Xcode dev ビルド**（`expo run:ios`）のみ
  - 手順（App Store Production 配信後）:
    1. 本番ビルドを実機で取得
    2. レポートを 4 件作成し、それぞれで PDF 出力を成功させる
    3. 4 回目の直後にダイアログが表示されることを確認（Apple の年間 3 回上限内）
...
- **iOS TestFlight 制限（Issue #289 で再発見）**: `SKStoreReviewController.requestReview()` は TestFlight 配布アプリでは**絶対にダイアログを表示しない**。これは Apple の意図的な仕様で、アプリコードで回避する手段はない。検証は App Store Production 版または Xcode dev ビルドでのみ可能。この制限は `docs/how-to/workflow/ios_testflight_release.md` と `docs/reference/lessons.md` にも恒久的に記録している。
- **計上タイミング変更（ADR-0014）**: 本 ADR の「累計 PDF 出力 4 回目 / 20 回目で発火」という条件は、ADR-0014 で計上ポリシーが「PDF 生成成功 = 1 回」に変わったあとも意味論は同じ。`maybeRequestReview` が受け取る `cumulativeCount` は `recordExport` 直後の `countAllExports()` の値なので、閾値判定ロジックは変更不要。
