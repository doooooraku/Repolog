...
- Issue: #286
- PR: （マージ後に追記）
- 関連 ADR: ADR-0002 (PDF フォント), ADR-0011 (レイアウト無料開放), **ADR-0017 (本 ADR の前提条件回復)**
...
### 2026-04-08 補遺（Re-evaluation, **重要**）

本 ADR の slack 設計は **「`.photo-slot` 内の in-flow 縦消費がゼロである」** という暗黙の前提に依存している。すなわち SSoT (`docs/reference/pdf_template.md`) の通り `.photo-no` が `.photo-frame` の **内側に absolute** で配置されていることが条件。

**この前提が PR #212 (commit `963b7c7`, 2026-03-23) の構造変更で密かに破壊されていた**:

- PR #212 は `.photo-no` を `.photo-frame` の外に出し、`.photo-slot` の flex 子として in-flow に組み込んだ
- これにより 1 スロットあたり ~3.82mm の縦消費が発生（standard 7.64mm / large 3.82mm）
- 本 ADR の 3.265mm slack budget では吸収しきれず、**iOS WebKit subpixel overflow による空白ページ挿入が完全に再発**（2026-04-08 ユーザー報告）

ADR-0009 起票時にこのドリフトを見落とした原因:
1. SSoT (`pdf_template.md`) と現コードの diff チェックを実施していなかった
2. `pdfTemplate.test.ts` が section 数しか見ておらず、`.photo-slot` の内部構造を assert していなかった

**復元対応**: ADR-0017 により `.photo-no` を `.photo-frame` の内側 absolute に戻し、CI で「`.photo-no` は `.photo-frame` の内側」を強制する構造不変条件テストを追加した。本 ADR の slack 計算は**そのまま正しく機能する状態に回復**した。

**後続改修者への警告**: `.photo-no` / `.photo-caption` / その他 `.photo-slot` 内要素を変更する際は、必ず `.photo-slot` の **in-flow 縦消費がゼロであるか** を確認すること。in-flow 要素を追加する場合は本 ADR の slack 予算（3.265mm）を超えないことを iOS 実機で検証すること。

### 2026-04-09 補遺（iOS 26 で再発 → ADR-0018 に拡張）

本 ADR の **「1mm slack で iOS の subpixel 丸め (~0.5mm) を吸収する」** という設計は **iOS 18 までの挙動を前提**としていた。iOS 26.2.1 で同種の空白ページ挿入が再発（2026-04-09 ユーザー報告）し、PyMuPDF 実測の結果、iOS 26 の WKWebView 印刷エンジンが **`break-after: page` 後の `.page` を物理ページの y=54.2pt（≒19mm）から配置する** quirk を発見した。

この offset は iOS 18 時代の「subpixel 丸め (0.5mm 未満)」という前提を **38 倍** 上回っており、本 ADR の 1mm slack では完全に飲み込まれた。**結果として表紙以外の全写真ページでフッターが次の物理ページに押し出され、+5 ページの空白挿入を生んだ**（standard 10 写真で実測: 期待 6 ページ → 実体 11 ページ）。

**対応**: ADR-0018 により `.page { height: calc(var(--page-h) - 1mm) }` を `calc(var(--page-h) - 20mm)` に拡大した。20mm の内訳は観測値 19mm + 安全マージン 1mm。本 ADR の **slack 設計思想（`.page` を `@page` より物理的に小さくする）** は依然として正しく、ADR-0018 はその数値を iOS 26 の挙動に合わせて拡大した補強である。本 ADR は引き続き `Accepted` であり、`Superseded` にはしない。両 ADR は積層的に機能する。

**ADR-0009 起票時にこの可能性を想定できなかった原因**:
1. iOS 印刷エンジンの挙動が OS メジャー更新で「桁違い」に変わる可能性が、設計時の脅威モデルに含まれていなかった
2. 本 ADR の Re-evaluation criteria は「ユーザー報告があれば 1mm を 2mm に増やす」とだけ書かれており、桁違いの拡大には対応していなかった
3. iOS バージョンアップを「PDF 回帰検証のトリガー」として運用に組み込めていなかった

**後続改修者への警告**: 本 ADR の slack 設計は「OS バージョン横断で挙動が一定」を暗黙の前提としていたが、それは破られた。**iOS メジャー更新時は必ず手動 24 ケース回帰検証** （ADR-0018 の Acceptance 参照）を実施すること。
