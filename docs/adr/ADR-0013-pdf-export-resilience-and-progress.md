...
- **2026-04-09 補遺: 進捗バーは UI 層で monotonic non-decreasing にクランプする**。`pdfTemplate.ts:521` の `input.onProgress?.(0, input.photos.length)` は初回 attempt で「即 0% 表示」を実現するために必要だが、フォールバックチェーン下では attempt 2 以降でも同じ行が発火し、UI 側の進捗バーが 80% → 0% に逆戻りしてしまう（ユーザー報告）。`app/reports/[id]/pdf.tsx` の `handleExport` 内で `setExportProgress((prev) => prev == null ? next : Math.max(prev, next))` として functional setState でクランプすることで、service 層の契約を一切変えずに解決している。テスト: `__tests__/pdfExportProgressClamp.test.ts`（7 ケース）。なお「attempt 1 が常に失敗する」という別の異常が背後にある疑いがあり、Follow-up Issue で根本調査する（本 ADR のスコープ外）
- **2026-04-10 補遺（Issue #296）: 擬似進捗タイマーの起動タイミングを前倒しする**。本 ADR Decision 5 で定義した「80-95% の印刷フェーズ擬似タイマー」が、**実装上は `await generatePdfFile()` の後に起動されていた**ため、印刷フェーズ（= `Print.printToFileAsync` 実行中）にはタイマーが走っておらず、ユーザーから見ると「80% で 10 数秒完全停止」に見えていた（写真 10 枚 + キャプション付きシナリオで再現）。`generatePdfFile` は `buildPdfHtml`（写真処理）+ `printHtml`（印刷）の 2 段を直列実行し、resolve 時点で印刷フェーズは既に終了している。修正として `handleExport` 内に `startPrintPhaseTimer(photoCount)` ヘルパーを抽出し、**onProgress コールバック内で `processed >= total && total > 0` を検知した瞬間に起動する**。`generatePdfFile` 後の呼び出しは 0 枚レポート・例外経路用の安全ネットとして残し、二重起動ガードで冪等にした。tick 間隔も `Math.max(200, photoCount * 5)` → `Math.max(500, photoCount * 30)` に保守化（10 枚 ≒ 7.5s / 40 枚 ≒ 18s / 70 枚 ≒ 31.5s で 95% 到達）し、95% 停滞も同時に軽減した。テスト: `__tests__/pdfExportProgressClamp.test.ts` に起動条件 4 ケース + tick 間隔 3 ケース + tick 進行 4 ケース + integration 1 ケースを追加。**設計原則**: 擬似進捗タイマーは「監視対象フェーズの開始と同期して起動する」こと（対象フェーズ終了後に起動しては意味がない）
- **2026-04-11 補遺（Issue #298）: プログレスバー廃止と attempt 1 タイムアウト 10 秒キャップ**。PR #297 の実機検証で、標準レイアウト + 写真 10 枚のシナリオで **attempt 1 (full quality) が `Print.printToFileAsync` で 40 秒 hang する新バグ** が判明。PR #297 の擬似タイマーは 80→95% を 7.5 秒で到達させた後、残り約 30 秒を 95% で停滞するため、結果として「80% 停滞」が「95% 停滞」に移動しただけで UX は根本改善していなかった。根本原因（`photo-grid.two` の CSS / HTML サイズ閾値 / expo-print 境界条件のいずれか）は Issue #298 で継続調査する前提で、**本補遺では 2 つの意思決定を記録する**: **(1) Decision 5 の単一プログレスバーを廃止** し、ActivityIndicator + 静的メッセージ (`t.pdfGenerating`) に置換。理由は「写真処理 0-80% / 印刷 80-95% / 完了 100%」という多層フェーズ管理の実装コストが、実際の UX 改善に対して割に合わないため（hang 発火時の 95% 停滞も解消不能）。**(2) `pdfService.ts` の動的タイムアウトを attempt-aware に変更** し、attempt 1 のみ上限 10 秒でキャップ (`ATTEMPT_1_TIMEOUT_CAP_MS = 10_000`)。正常経路の attempt 1 は写真 10 枚でも 1 秒以内に完了する実測値に対して 10 倍以上のマージンがあり、hang 時は 10 秒で打ち切り reduced preset へフォールバック。attempt 2 以降は従来通り `30s + 1s × photoCount` の動的計算を維持（安全網として時間を確保）。**待機時間の実効**: Issue #298 シナリオで 44 秒 → 約 12 秒に短縮（attempt 1 cap 10s + fallback cooldown 0.3s + attempt 2 完了 0.6s + SAF ピッカーまで）。**削除したもの**: `exportProgress` state、`startPrintPhaseTimer`/`stopPrintPhaseTimer`、擬似タイマー setInterval、monotonic clamp、`onProgress` callback（pdfService / pdfTemplate から）、`pdfGeneratingProgress` i18n キー（19 言語）、`progressContent`/`progressText`/`progressTrack`/`progressFill` styles、`pdfExportProgressClamp.test.ts`（全 19 ケース）。**保持したもの**: 動的タイムアウト機構（attempt 2+）、フォールバックチェーン（full → reduced → tiny）、ストレージ事前チェック、メモリ強制解放（`Image.clearMemoryCache()` + 350ms 待機）、ref-based 二重発火ガード、出力成功後のホーム遷移（`router.dismissAll()`）。Decision 5 と 6 は本補遺により **Superseded**、Decision 1〜4 は維持
